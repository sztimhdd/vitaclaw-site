      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4">
            点击流 <span className="text-muted px-2">&rarr;</span> 对话流
          </h2>
          <p className="text-muted text-lg max-w-3xl mx-auto">
            一行代码不改，ChatKit SDK 让老旧系统秒变 AI 驱动，Agent 自动完成跨系统数据回填
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-w-5xl mx-auto">
          ...
          <div className="relative rounded-2xl border border-card-border bg-card overflow-hidden hover:border-accent/30 transition-all duration-300">
            <div className="border-b border-card-border bg-black/40 px-5 py-3 flex items-center justify-between">
              ...
              <span className="text-xs text-muted font-mono">传统业务系统 · 银行 ERP</span>
              <div className="w-12" />
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between items-center pt-2 border-t border-card-border">
                <span className="text-xs text-muted/50">共 38 条记录</span>
                <div className="flex gap-1">
                  {["1", "2", "3", "...", "8"].map((p) => (
                    <span key={p} className={`w-6 h-6 rounded flex items-center justify-center text-xs ${p === "1" ? "bg-accent/20 text-accent" : "text-muted/50"}`}>{p}</span>
                  ))}
                </div>
              </div>
              {/* Overlay "masked data" badge */}
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex-1">
                  <div className="rounded-2xl rounded-tl-sm bg-accent/10 border border-accent/20 px-4 py-3">
                    <p className="text-sm text-foreground">
                      您好！已为您查询到客户 <span className="text-accent font-mono">***</span> 的信息：
                    </p>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1">
                  <div className="rounded-2xl rounded-tl-sm bg-accent/10 border border-accent/20 px-4 py-3">
                    <p className="text-sm text-foreground">
                      &#x2705; 复核单已自动生成，已发送至审批人 <span className="text-accent font-mono">张经理</span> 待办事项
                    </p>
                    <div className="mt-2 flex items-center gap-2 text-xs text-muted">
                      <span className="inline-flex items-center gap-1 rounded-md border border-green-500/20 bg-green-500/10 px-2 py-0.5 text-green-400">
                        自动回填
                      </span>
                      <span>耗时 0.3s</span>
                    </div>
